/*Ejercicio 2
Para este ejercicio utilice los métodos filter, map, y reducepara generar algunos informes.

Cree un cars.js archivo con el siguiente contenido:
*/
const cars =
...
const ferraris = cars.filter(function (ca){
    return ca.brand === "Ferrari"
})
console.log(ferraris)

let nineties = cars.filter(function (mo) {
    return mo.model >= 1980 && mo.model <= 1990;
})
console.log(nineties);

let list = cars.map(function (arr) {
    return `This ${arr.brand} from ${arr.model} cost ${arr.price}`;
})
console.log(list);
